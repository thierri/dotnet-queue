...
namespace queueHandler
{
    ...
    public class RedisJobQueue
    {
        ...
        public event EventHandler<contractProcessor> OnJobReceived;

        public RedisJobQueue(Lazy<ConnectionMultiplexer> multiplexer, string jobName, CancellationToken cancellationToken = new CancellationToken())
        {
        ...
        private async Task HandleNewJobs()
        {
            ...
            {
                // Fire the Event
                OnJobReceived?.Invoke(this, new contractProcessor());
        ...
        public void AddJobAsync(RedisValue job)
        {
            ...
            ConnectionMultiplexer.GetSubscriber().PublishAsync(_subChannel, "");
        }
        ...
        /// <returns></returns>
        public void AddJobAsync(Dictionary<RedisValue, RedisValue> parametersDictionary)
        {
            ...
            var db = Database;
            var id = db.StringIncrement($"{_jobName}:jobid");
            var key = $"{_jobName}:{id}";

            db.HashSet(key, parametersDictionary.Select(entries => new HashEntry(entries.Key, entries.Value)).ToArray());

            db.ListLeftPush(_jobQueue, key);
            ConnectionMultiplexer.GetSubscriber().Publish(_subChannel, "");
        }
    }
}